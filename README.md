# Authsignal Terraform multi-brand example
...
The repository configures Brand One and Brand Two across `dev`, `test`, `preprod`, and `prod`. Each of the eight brand/environment pairs is a Terraform root under `live/`, with its own tenant, Management API credential, backend key, state, and provider lock file. Only real deployable roots live under `live/`.
